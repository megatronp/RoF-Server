...
switch (_code) do
{
	...
	case 18:
	{
		...
		if(_shift && (playerSide == west) && (uniform player == "U_Rangemaster") || (uniform player == "U_B_CombatUniform_mcam_worn")) then
		{
	...
	*/
    ...
    case 37:
    {
        if (!_shift && !_alt && !_ctrlKey && (playerSide == west) && (vehicle player != player && (typeOf vehicle player) in ["B_Heli_Light_01_armed_F"])) then
        {
    ...
    //Robbing
		...
		{
			if((animationState cursorTarget) != "Incapacitated" && (currentWeapon player == primaryWeapon player OR currentWeapon player == handgunWeapon player) && currentWeapon player != "" && !life_knockout && (!(player getVariable["restrained",false]) OR !(player getVariable["restrainedCiv",false])) && !life_istazed) then
			{
	...
	case 34:
	{
		...
		if (_shift) then
		{
			if (vehicle player == player && (animationState player) != "Incapacitated" && !life_istazed && (!(player getVariable ["restrainedCiv", false]) OR !(player getVariable ["restrained", false]))) then
			{
			...
			if(_veh isKindOf "House_F" && playerSide == civilian) then {
				...
				if(_veh in life_vehicles && player distance _veh < 8) then {
					if(_locked == 2) then {
						...
						player say3D "unlock";
						if(typeOf _veh in (__GETC__(Amarok_RestrictSling))) then
						{

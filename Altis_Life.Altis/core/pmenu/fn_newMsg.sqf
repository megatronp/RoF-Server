...
switch(_type) do
{
	case 0:
	{
		...
		ctrlSetText[88886, format["Message To: %1",life_smartphoneTarget getVariable["realname",name life_smartphoneTarget]]];
	...
	case 1:
	{
		...
		hint format["You sent %1 a message: %2",life_smartphoneTarget getVariable["realname",name life_smartphoneTarget],_msg];	
		ctrlShow[88885, true];
		closeDialog 88883;
	};
	//copmessage
	case 2:
	{
		if(({side _x == west} count playableUnits) == 0) exitWith {hint format["Currently there are no cops on duty. Please try it again later."];};
	...
	case 5:
	{
		...
		if(isNULL life_smartphoneTarget) exitWith {hint format["No person is selected!"];};
		...
		hint format["Admin Message Sent To: %1 - Message: %2",life_smartphoneTarget getVariable["realname",name life_smartphoneTarget],_msg];
		closeDialog 88883;
	};
	//emergencyloading
	case 6:

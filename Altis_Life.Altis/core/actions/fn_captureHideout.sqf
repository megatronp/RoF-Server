...
_hideout = (nearestObjects[getPosATL player,["Land_u_Barracks_V2_F","Land_i_Barracks_V2_F","Land_BagBunker_Large_F"],50]) select 0;
...
if(!isNull _group) then {
	...
	_cpRate = 0.0045;
	[[1,format[localize "STR_GNOTF_AttCapture",name player,(group player) getVariable "gang_name"]],"life_fnc_broadcast",true,false] spawn life_fnc_MP;
} else {
	_cpRate = 0.0075;
};
...
if((player getVariable["restrained",false]) OR (player getVariable["restrainedCiv",false])) exitWith {life_action_inUse = false;_hideout setVariable["inCapture",false,true];_ui = "osefStatusBar" call BIS_fnc_rscLayer;_ui cutRsc["osefStatusBar","PLAIN"];};
...
//_gangName = _group getVariable ["gang_name",""];
[[1,format[localize "STR_GNOTF_CaptureSuccess",name player,(group player) getVariable "gang_name" ]],"life_fnc_broadcast",true,false] spawn life_fnc_MP;

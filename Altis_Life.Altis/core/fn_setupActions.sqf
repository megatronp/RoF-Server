...
switch (playerSide) do
{
	case civilian:
	{
		...
		//Charge Battery
		life_actions = life_actions + [player addAction["Charge Battery",life_fnc_batteryChargeCar,"",0,false,false,"",' life_battery < 50 && (vehicle player isKindOf "Car") ']];
	};
	
	case independent:
	{
		life_actions = life_actions + [player addAction["<t color='#00FF00'>Kidney Operation</t>",life_fnc_kidneyOperation,"",1,false,true,"",' playerSide == independent && !isNull cursorTarget && player distance cursorTarget < 3.5 && cursorTarget isKindOf "Man" ']];
	};
};

...
if(_shooter isKindOf "Man" && alive player) then
{
	if(!life_istazed) then
	{
		...
		sleep 4;
		[[player,"Acts_UnconsciousStandUp_part1"],"life_fnc_animSync",true,false] spawn life_fnc_MP;
		if((goggles _unit) in _goggles) then 
		{
			[[0,format[localize "STR_NOTF_TazedMask",_shooter getVariable["realname",name _shooter]]],"life_fnc_broadcast",west,false] spawn life_fnc_MP;
		} else {
			[[0,format[localize "STR_NOTF_Tazed", _unit getVariable["realname",name _unit], _shooter getVariable["realname",name _shooter]]],"life_fnc_broadcast",west,false] spawn life_fnc_MP;
		};
